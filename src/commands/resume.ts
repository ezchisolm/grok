...
const resume: BotCommand = {
  ...
  async execute(interaction, { music }) {
    ...
    if (!controller.nowPlaying) {
      await interaction.reply({ content: "Nothing is playing to resume.", flags: MessageFlags.Ephemeral });
      return;
    }

    if (controller.status === AudioPlayerStatus.Playing) {
      await interaction.reply({ content: "Playback is already running.", flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    if (!success) {
      await interaction.reply({ content: "Sorry, I couldn't resume playback.", flags: MessageFlags.Ephemeral });
      return;
    }
